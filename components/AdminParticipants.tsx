...
import { Plus, Trash2, UserPlus, Database, Edit2, Check, X, User } from 'lucide-react';
...
const AdminParticipants: React.FC<Props> = ({ participants, setParticipants, entries, setEntries }) => {
  ...
  const addParticipant = (e: React.FormEvent) => {
    ...
    if (entryNums.length > 3) { alert("Máximo 3 inscrições."); return; }
  ...
  const removeParticipant = (id: string) => {
    if (!confirm("Remover participante e suas inscrições?")) return;
  ...
  // Ordenar participantes por data de cadastro (mais recentes primeiro)
          ...
          <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl sticky top-24">
            ...
            <form onSubmit={addParticipant} className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none transition-all" placeholder="Ex: Baianinho" required />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase text-slate-500 mb-1.5">Números das Bolas (1-200)</label>
                <input type="text" value={entryInput} onChange={(e) => setEntryInput(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-emerald-500 outline-none transition-all" placeholder="Ex: 7, 22, 45" required />
              </div>
          ...
          <div className="grid gap-3 max-h-[600px] overflow-y-auto pr-2">
            {sortedParticipants.length === 0 ? (
              ...
              sortedParticipants.map(p => (
                <div key={p.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex justify-between items-center group hover:border-slate-700 transition-all shadow-sm">
                  <div className="flex-1">
                    {editingId === p.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={saveEdit} className="p-1 text-emerald-400 hover:bg-emerald-400/10 rounded transition-colors"><Check className="w-4 h-4" /></button>
                        <button onClick={() => setEditingId(null)} className="p-1 text-red-400 hover:bg-red-400/10 rounded transition-colors"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1">
                          {p.entryNumbers.map(num => (
                            <span key={num} className="text-[10px] font-black text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">#{num}</span>
                          ))}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="flex gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => removeParticipant(p.id)} className="p-2 text-slate-400 hover:text-red-400 rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ))
            )}
